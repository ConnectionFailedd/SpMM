...
int main() {
    ...
    for(MKL_INT count; count < nnz; count++) {
    ...
    mkl_sparse_d_export_csr(csr_matrix, &indexing, &rows, &columns, &csrRowPointers, &csrRowPointers1, &csrColumnIndices, &csrValues);

    spmm_csr_dense(m, n, p, csrValues, csrColumnIndices, csrRowPointers, nullptr, nullptr);
}